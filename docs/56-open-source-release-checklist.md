...
- [ ] 启用私密安全报告，并在 SECURITY.md 填写联系方式。
- [x] 给当前 Adapter 设置真实维护者 @wzn7423162；仍维持 `beta`，直至 Windows 实机和平台实页验收。
- [ ] 创建首个版本标签及变更说明，明确只有拼多多女装 T 恤 MVP 可运行。
- [x] 已准备 [v0.1.2 Beta 发布说明](58-v0.1.2-release-notes.md)，等待 GitHub 仓库与正式 Release。
...
- [x] 已配置 CI：manifest、单元测试、MVP 测试和 TypeScript 构建；首次推送后需确认托管运行结果。干净目录 `npm ci` 审计为 0 个已知漏洞。
