...
import {parseTabularInput} from './tabular-input.ts'
...
export function pastePrices(form:FormData,text:string):PasteResult{
 const {headers,rows}=parseTabularInput(text),aliases:Record<string,string>={'颜色':'color','颜色分类':'color','尺码':'size','拼单价':'groupPrice','拼单价(元)':'groupPrice','单买价':'singlePrice','单买价(元)':'singlePrice','库存':'stock','库存(件)':'stock'}
...
export function pasteMeasurements(form:FormData,text:string):PasteResult{
 const {headers,rows}=parseTabularInput(text);if(headers.filter(h=>h==='尺码').length!==1)throw new Error('表头必须包含尺码')
