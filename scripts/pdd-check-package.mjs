...
const args=process.argv.slice(2), [file,contextFile]=args
if (!file || args.length>2 || args.some(a=>a.startsWith('--'))) throw new Error('用法：npm run pdd:check-package -- <商品.json> [核验配置.json] ；默认只读预检')
const input=JSON.parse(await readFile(resolve(file),'utf8'))
const options=contextFile?JSON.parse(await readFile(resolve(contextFile),'utf8')):{}
if(!options||Array.isArray(options)||typeof options!=='object'||Object.keys(options).some(k=>!['freightProfiles','mediaReceipts','shopBindings'].includes(k)))throw new Error('READBACK_CONTEXT_INVALID')
const plan=compileProductPackage(input,options)
const dir=resolve('output/playwright/real-package-readback',new Date().toISOString().replaceAll(':','-'))
...
try {
  ...
  console.log(JSON.stringify({mode:'独立商品回读',plan},null,2))
  await terminal.question('请在独立窗口登录并进入指定店铺的新建 T 恤编辑页，并填写与计划相符的货号，请展开服务区供只读核验。准备好后按 Enter：')
  ...
  const result=await readbackProductPackage(targets[0],input,options)
  await writeFile(`${dir}/report.json`,JSON.stringify(result,null,2))
  if(result.status!=='full_product_verified')process.exitCode=2
