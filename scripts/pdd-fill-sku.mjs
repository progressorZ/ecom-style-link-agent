...
import { executeSkuPlan, compileSkuPlan, compileSkuPlanFromPackage } from './pdd-sku-adapter.mjs'
const args=process.argv.slice(2), file=args.find(arg=>!arg.startsWith('--'))
if (!file || args.some(arg=>arg.startsWith('--') && arg!=='--apply')) throw new Error('用法：npm run pdd:fill-sku -- <计划.json> [--apply]；默认仅预检')
const input=JSON.parse(await readFile(resolve(file),'utf8'))
const plan=input.schemaVersion ? compileSkuPlanFromPackage(input) : compileSkuPlan(input)
const dir=resolve('output/real-sku-fill',new Date().toISOString().replaceAll(':','-'))
...
try {
  ...
  console.log(JSON.stringify({mode:args.includes('--apply')?'填写后回读':'只读预检',plan},null,2))
  await terminal.question('请在独立窗口登录并进入指定店铺的新建 T 恤编辑页，并准备与计划相符的货号及完整颜色/尺码组合。准备好后按 Enter：')
  ...
  const result=await executeSkuPlan(targets[0],plan,{dryRun:!args.includes('--apply')})
