...
if (!file || args.some(arg=>arg.startsWith('--') && arg!=='--apply')) throw new Error('用法：npm run pdd:fill-size-sync -- <计划.json>  [--apply]；默认只读预检')
const input=JSON.parse(await readFile(resolve(file),'utf8'))
const plan=compileSizeSyncPlan(input)
const dir=resolve('output/playwright/real-size-sync',new Date().toISOString().replaceAll(':','-'))
...
try {
  ...
  console.log(JSON.stringify({mode:args.includes('--apply')?'填写尺码表详情同步并回读':'只读预检',plan},null,2))
  await terminal.question('请在独立窗口登录并进入指定店铺的新建 T 恤编辑页，并填写与计划相符的货号，仅按明确的布尔值设置同步状态。准备好后按 Enter：')
  ...
  const result=await executeSizeSync(targets[0],input,{dryRun:!args.includes('--apply')})
