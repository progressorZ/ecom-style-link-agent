...
import { executeSkuImages, compileSkuImagePlan } from './pdd-sku-image-adapter.mjs'
const args=process.argv.slice(2), file=args.find(arg=>!arg.startsWith('--'))
if (!file || args.some(arg=>arg.startsWith('--') && arg!=='--apply')) throw new Error('用法：npm run pdd:upload-sku-images -- <计划.json>  [--apply]；默认只读预检')
const input=JSON.parse(await readFile(resolve(file),'utf8'))
const plan=compileSkuImagePlan(input)
const dir=resolve('output/playwright/real-sku-images',new Date().toISOString().replaceAll(':','-'))
...
try {
  ...
  console.log(JSON.stringify({mode:args.includes('--apply')?'上传图片并回读':'只读预检',plan},null,2))
  await terminal.question('请在独立窗口登录并进入指定店铺的新建 T 恤编辑页，并填写与计划相符的货号，准备与计划绑定一致的颜色尺码，目标 SKU 预览图保持为空，图片路径相对运行目录解析。准备好后按 Enter：')
  ...
  const result=await executeSkuImages(targets[0],input,{dryRun:!args.includes('--apply')})
