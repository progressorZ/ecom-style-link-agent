...
import { executePricing, compilePricingPlan } from './pdd-pricing.mjs'
const args=process.argv.slice(2), file=args.find(arg=>!arg.startsWith('--'))
if (!file || args.some(arg=>arg.startsWith('--') && arg!=='--apply')) throw new Error('用法：npm run pdd:fill-pricing -- <计划.json>  [--apply]；默认只读预检')
const input=JSON.parse(await readFile(resolve(file),'utf8'))
const plan=compilePricingPlan(input)
const dir=resolve('output/playwright/real-pricing',new Date().toISOString().replaceAll(':','-'))
...
try {
  ...
  console.log(JSON.stringify({mode:args.includes('--apply')?'填写参考价与满件折扣并回读':'只读预检',plan},null,2))
  await terminal.question('请在独立窗口登录并进入指定店铺的新建 T 恤编辑页，并填写与计划相符的货号，仅使用已确认的测试价格、折扣和 SKU。准备好后按 Enter：')
  ...
  const result=await executePricing(targets[0],input,{dryRun:!args.includes('--apply')})
