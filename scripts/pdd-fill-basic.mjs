...
import { executeBasicPlan, compileBasicPlan } from './pdd-basic-adapter.mjs'
const args=process.argv.slice(2), file=args.find(arg=>!arg.startsWith('--'))
if (!file || args.some(arg=>arg.startsWith('--') && arg!=='--apply')) throw new Error('用法：npm run pdd:fill-basic -- <计划.json> [--apply]；默认仅预检')
const plan=compileBasicPlan(JSON.parse(await readFile(resolve(file),'utf8')))
const dir=resolve('output/real-basic-fill',new Date().toISOString().replaceAll(':','-'))
...
try {
  ...
  await terminal.question('请在独立窗口登录并进入指定店铺的新建 T 恤编辑页。准备好后按 Enter：')
  ...
  const result=await executeBasicPlan(targets[0],plan,{dryRun:!args.includes('--apply')})
