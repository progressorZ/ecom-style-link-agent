...
if (!file || args.some(arg=>arg.startsWith('--') && arg!=='--apply')) throw new Error('用法：npm run pdd:upload-images -- <计划.json>  [--apply]；默认只读预检')
const input=JSON.parse(await readFile(resolve(file),'utf8'))
const plan=compileImagePlan(input)
const dir=resolve('output/playwright/real-carousel',new Date().toISOString().replaceAll(':','-'))
...
try {
  ...
  await terminal.question('请在独立窗口登录并进入指定店铺的新建 T 恤编辑页，并填写与计划相符的货号，保持商品轮播图区为空，图片路径相对运行目录解析。准备好后按 Enter：')
  ...
  const result=await executeCarousel(targets[0],input,{dryRun:!args.includes('--apply')})
