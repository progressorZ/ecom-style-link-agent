...
import { executeSizePlan, compileSizePlan } from './pdd-size-adapter.mjs'
const args=process.argv.slice(2), file=args.find(arg=>!arg.startsWith('--'))
if (!file || args.some(arg=>arg.startsWith('--') && arg!=='--apply')) throw new Error('用法：npm run pdd:fill-size -- <计划.json> [--apply]；默认仅预检')
const input=JSON.parse(await readFile(resolve(file),'utf8'))
const plan=compileSizePlan(input)
const dir=resolve('output/real-size-fill',new Date().toISOString().replaceAll(':','-'))
...
try {
  ...
  await terminal.question('请在独立窗口登录并进入指定店铺的新建 T 恤编辑页，并填写与计划相符的货号，准备好与计划相符的颜色尺码矩阵，尺码表仅支持肩宽/胸围的单值厘米数据。准备好后按 Enter：')
  ...
  const result=await executeSizePlan(targets[0],input,{dryRun:!args.includes('--apply')})
