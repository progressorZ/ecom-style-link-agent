...
import { prepareSkuMatrix, compileMatrixPlan } from './pdd-matrix-adapter.mjs'
const args=process.argv.slice(2), file=args.find(arg=>!arg.startsWith('--'))
if (!file || args.some(arg=>arg.startsWith('--') && arg!=='--apply')) throw new Error('用法：npm run pdd:prepare-skus -- <计划.json> [--apply]；默认仅预检')
const input=JSON.parse(await readFile(resolve(file),'utf8'))
const plan=compileMatrixPlan(input)
const dir=resolve('output/real-matrix-fill',new Date().toISOString().replaceAll(':','-'))
...
try {
  ...
  await terminal.question('请在独立窗口登录并进入指定店铺的新建 T 恤编辑页，并填写与计划相符的货号，使用空白规格区或完全一致的已有矩阵。准备好后按 Enter：')
  ...
  const result=await prepareSkuMatrix(targets[0],input,{dryRun:!args.includes('--apply')})
