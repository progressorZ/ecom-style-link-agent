...
import { verifyServices, compileServicesPlan } from './pdd-services.mjs'
const args=process.argv.slice(2), file=args.find(arg=>!arg.startsWith('--'))
if (!file || args.length!==1) throw new Error('用法：npm run pdd:check-services -- <计划.json> ；默认只读预检')
const input=JSON.parse(await readFile(resolve(file),'utf8'))
const plan=compileServicesPlan(input)
const dir=resolve('output/playwright/real-services',new Date().toISOString().replaceAll(':','-'))
...
try {
  ...
  console.log(JSON.stringify({mode:'只读服务核验',plan},null,2))
  ...
  const result=await verifyServices(targets[0],input)
  await writeFile(`${dir}/report.json`,JSON.stringify(result,null,2))
  if(result.status!=='services_subset_verified')process.exitCode=2
