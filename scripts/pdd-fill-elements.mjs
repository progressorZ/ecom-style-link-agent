...
import { executeElements, compileElementsPlan } from './pdd-elements.mjs'
const args=process.argv.slice(2), file=args.find(arg=>!arg.startsWith('--'))
if (!file || args.some(arg=>arg.startsWith('--') && arg!=='--apply')) throw new Error('用法：npm run pdd:fill-elements -- <计划.json>  [--apply]；默认只读预检')
const input=JSON.parse(await readFile(resolve(file),'utf8'))
const plan=compileElementsPlan(input)
const dir=resolve('output/playwright/real-elements',new Date().toISOString().replaceAll(':','-'))
...
try {
  ...
  await terminal.question('请在独立窗口登录并进入指定店铺的新建 T 恤编辑页，并填写与计划相符的货号，仅填写明确指定的流行元素完整集合。准备好后按 Enter：')
  ...
  const result=await executeElements(targets[0],input,{dryRun:!args.includes('--apply')})
